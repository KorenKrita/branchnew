...
| **Ghostty** | `ghostty-fork` 脚本 + Hammerspoon 全局热键 | 需装 [Hammerspoon](https://www.hammerspoon.org/);Ghostty tip/1.4+ 精确匹配 TTY,1.3.1 按 cwd 匹配 |
...
3. 写 `~/.hammerspoon/init.lua`:
   ```lua
   -- ⌘F = split right, ⌘⇧F = split down
...
| **Ghostty** | `ghostty-fork` script + Hammerspoon global hotkey | requires [Hammerspoon](https://www.hammerspoon.org/); Ghostty tip/1.4+ uses precise TTY matching, 1.3.1 matches by cwd |
